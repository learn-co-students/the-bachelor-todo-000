...
#solution and study for refactoring
=begin 
...
=end
  ...
  data.each do |season_key, contestant_array| #all the data
    if season_key == season
      contestant_array.each do |array_index| #contestants array
        array_index.each do |key, value|    
          if array_index["status"] == "Winner"
            first_name = array_index["name"].split(" ") #only need first name
            return first_name[0]
          end
        end
      end
...
def get_contestant_name(data, occupation)
  ...
  data.each do |season_key, contestant_array|
    contestant_array.each do |array_index|
      array_index.each do |key, value|
        
        if array_index["occupation"] == occupation
          return array_index["name"]
        end
      end
    end
  end
...
def get_occupation(data, hometown)
  ...
  data.each do |season_key, contestant_array|
    contestant_array.each do |array_index|
      array_index.each do |key, value|
        if array_index["hometown"] == hometown
          return array_index["occupation"]
        end
      end
    end
  end
...
def get_average_age_for_season(data, season)
  ...
  data.each do |season_key, contestant_array| #all the data
    if season_key == season
      contestant_array.each do |array_index| #contestants array
        array_index.each do |key, value|
          age_array << array_index["age"].to_f
        end
      end
    end
  end

  #Used the 0.0 in the inject method so it would use floats
  average_age = age_array.inject(0.0) {|sum, eval| sum + eval} / age_array.count
  average_age.round
end